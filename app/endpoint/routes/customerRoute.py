...
def show_by_id(id: int, session: Session = Depends(get_session)):
    customer = session.exec(select(Customer).where(Customer.id == id, Customer.status)).first()
    if not customer:
        raise HTTPException(status_code=404, detail="Customer with specified ID doesn't exist")
    return customer
...
def show_by_name(name: str, session: Session = Depends(get_session)):
    customer = session.exec(select(Customer).where(Customer.name == name, Customer.status)).first()
    if not customer:
        raise HTTPException(status_code=404, detail="Customer with specified NAME doesn't exist")
    return customer
...
def show_by_email(email: str, session: Session = Depends(get_session)):
    customer = session.exec(select(Customer).where(Customer.email == email, Customer.status)).first()
    if not customer:
        raise HTTPException(status_code=404, detail="Customer with specified EMAIL doesn't exist")
    return customer
