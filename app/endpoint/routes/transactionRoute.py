...
from app.endpoint.schemas.transactionSchema import PurchasePayload as CreateValidation
...
def purchase(
    ...
    owned_ids = {
        ...
        for ct in session.exec(
            select(CustomerTitle).where(CustomerTitle.customer_user_id == current.user_id)
        ).all()
    }
    ...
    for name in payload.titles:
        ...
        if not title:
            raise HTTPException(status_code=404, detail=f"Título '{name}' no encontrado o no disponible")
        if title.id in owned_ids:
            raise HTTPException(status_code=409, detail=f"Ya tienes '{name}' en tu biblioteca")
        titles_to_buy.append(title)

    def _final_price(t: Title) -> Decimal:
        if t.actual_discount == 0:
            return t.release_price
        return (t.release_price * (1 - Decimal(t.actual_discount) / 100)).quantize(Decimal("0.01"))
    ...
    transaction = Transaction(wallet_customer_user_id=current.user_id)
    ...
    for title in titles_to_buy:
        ...
        session.add(CustomerTitle(title_id=title.id, customer_user_id=current.user_id))
...
def history(
    ...
    transactions = session.exec(
        select(Transaction).where(Transaction.wallet_customer_user_id == current.user_id)
    ).all()
    ...
    for tx in transactions:
        ...
        for item in items:
            ...
            if title:
                purchased.append(PurchasedTitleItem(
                    name=title.name,
                    price_paid=item.price,
                    discount_applied=item.discount,
                ))
        result.append(TransactionHistoryItem(created_at=tx.created_at, titles=purchased))
    return result
