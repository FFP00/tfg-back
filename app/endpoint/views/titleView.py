...
def _developers(session: Session):
    return session.exec(
        select(Developer).join(User, Developer.user_id == User.id).order_by(User.name)
    ).all()
...
def index(request: Request, search: str = "", page: int = 1, session: Session = Depends(get_session)):
    ...
    if search:
        cond    = col(Title.name).ilike(f"%{search}%")
        q       = q.where(cond)
        count_q = count_q.where(cond)
    total  = session.exec(count_q).one()
    titles = session.exec(q.order_by(Title.created_at.desc()).offset((page - 1) * _PAGE).limit(_PAGE)).all()
...
def store(
    ...
    try:
        session.add(Title(
            ...
            developer_user_id = int(developer_user_id) if developer_user_id else None,
        ))
        session.commit()
        return RedirectResponse("/views/title/?success=Título+creado+correctamente", status_code=302)
...
def show(id: int, request: Request, session: Session = Depends(get_session)):
    title = session.get(Title, id)
    if not title:
        return RedirectResponse("/views/title/?error=Título+no+encontrado", status_code=302)
    media        = session.exec(select(Media).where(Media.title_id == id)).first()
    photo_fields = [f for f in _PHOTO_FIELDS if media and getattr(media, f)]
    has_trailer  = bool(media and media.trailer)
    dev_name     = title.developer.user.name if title.developer and title.developer.user else "—"
    return templates.TemplateResponse(request, "title/show.html", _ctx(request,
        title=title, photo_fields=photo_fields, has_trailer=has_trailer, dev_name=dev_name,
    ))
...
def update(
    ...
    try:
        ...
        title.developer_user_id = int(developer_user_id) if developer_user_id else None
        session.add(title)
        session.commit()
        return RedirectResponse(f"/views/title/{id}?success=Título+actualizado", status_code=302)
    except Exception as e:
        session.rollback()
        title = session.get(Title, id)
        return templates.TemplateResponse(request, "title/edit.html",
